...
#' 
## ------------------------------------------------------------------------
d2 <- readRDS( "cleaned-titles/categorized-titles.rds")
...
#' 
## ------------------------------------------------------------------------
Title.Test <- cbind( as.character( d2$TitleTxt ), 
                     d2$TitleTxt2, 
                     d2$TitleRole, 
                     d2$TitleResp ) %>% as.data.frame() 

colnames( Title.Test ) <- c( "Title", "Cleaned Title", "Role", "Responsibilty" )

set.seed( 100 )
ii <- sample(1:nrow(d2), 150 )
...
saveRDS( d2, "cleaned-titles/roles-and-responsibility.rds")
